...
class LeroiAirInterface:
    ...
    def conductivity_thicknesses(self) -> np.ndarray:
        ...
        layer_sigma = self.opts.layer_thicknesses * (1 / np.array(self.opts.layer_resistivities))
        plate_sigma = (
            [g.width for g in self.opts.plate_geometries]
            * (1 / np.array(self.opts.plate_resistivities))
        )
    ...
    def aliased_values(self) -> dict[str, Any]:
        ...
        return {
            ...
            "NLITH": 3,
            "GND_LVL": 0.0,
            "RES": self.resistivities,
            "SIG_T": self.conductivity_thicknesses,
            "RMU": np.ones_like(self.resistivities),
            "REPS": np.ones_like(self.resistivities),
            "CHRG": np.zeros_like(self.resistivities),
            "CTAU": np.zeros_like(self.resistivities),
            "CFREQ": np.ones_like(self.resistivities),
            "LITH": np.array([1, 2]) if self.opts.background.has_overburden else np.array([1]),
            "LITHP": 3 if self.opts.background.has_overburden else 2,
            "THICK": self.layer_properties[:, 1],
            "CELLW": self.opts.modelling.cell_size,
            "IPLATE": 1,
            "CNTR_East": self.plate.reference[0],
            "CNTR_North": self.plate.reference[1],
            "PLTOP": self.plate.reference[2],
            "PLNGTH": self.plate.strike_length,
            "DPWDTH": self.plate.dip_length,
            "DZM": self.plate.dip_direction,
            "DIP": self.plate.dip,
        }
    ...
    @property
    def record_3(self):
        return self.format_line(["ISW", "NSX", "STEP", "UNITS", "NCHNL", "KRXW", "OFFTIME"])
    ...
    @property
    def record_11(self):
        return self.format_multi_line(["RES", "SIG_T", "RMU", "REPS", "CHRG", "CTAU", "CFREQ"])
