...
def test_line_formatting(tmp_path):
    ...
    leroi_input = LeroiAirInput(opts=opts)

    line = leroi_input.format_line(["TXCLN", "CMP", "KPPM"])
    assert line == "0.0 3 0 \t ! TXCLN, CMP, KPPM"

    line = leroi_input.format_line("TMS")
    assert line == "2.3 2.6 3.2 \t ! TMS"

    multi_line = leroi_input.format_multi_line(["TMS", "WIDTH"])
...
def test_format_cfl_file(tmp_path):
    ...
    leroi_input = LeroiAirInput(opts=opts)
    cfl_str = leroi_input.format_cfl_file()
...
def test_slice_data_lines_returns_correct_station_rows(mock_output):
    lines = FAKE_OUT.splitlines()
    anchor = LeroiAirOutput._COMPONENT_ANCHORS["crossline"]
    rows = mock_output._slice_data_lines(lines, anchor)
    assert len(rows) == N_STATIONS
    assert rows[0].split()[4] == "1.1"
    assert rows[1].split()[4] == "4.4"


def test_extract_data_returns_channel_columns_only(mock_output, fake_outfile):
    data = mock_output._extract_data(fake_outfile, component="crossline")
    assert data.shape == (N_STATIONS, N_CHANNELS)


def test_extract_data_parses_transverse_component_values(mock_output, fake_outfile):
    data = mock_output._extract_data(fake_outfile, component="crossline")
    np.testing.assert_array_almost_equal(data[0], [1.1, 2.2, 3.3])
    np.testing.assert_array_almost_equal(data[1], [4.4, 5.5, 6.6])


def test_extract_data_parses_inline_component_values(mock_output, fake_outfile):
    data = mock_output._extract_data(fake_outfile, component="inline")
    np.testing.assert_array_almost_equal(data[0], [7.7, 8.8, 9.9])
    np.testing.assert_array_almost_equal(data[1], [10.0, 11.1, 12.2])


def test_extract_data_parses_vertical_component_values(mock_output, fake_outfile):
    data = mock_output._extract_data(fake_outfile, component="vertical")
    np.testing.assert_array_almost_equal(data[0], [13.3, 14.4, 15.5])
    np.testing.assert_array_almost_equal(data[1], [16.6, 17.7, 18.8])


def test_format_value_int(mock_input):
    assert mock_input._format_value(3) == "3"
    assert mock_input._format_value(np.int64(7)) == "7"


def test_format_value_float(mock_input):
    assert mock_input._format_value(1.5) == "1.5"
    assert mock_input._format_value(1.123456789) == "1.1235"
    mock_input.opts.float_precision = 2
    assert mock_input._format_value(3.141592653) == "3.14"
