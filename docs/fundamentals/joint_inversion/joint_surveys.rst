...
.. figure:: ../images/joint_surveys_ui.png
   ...
   User-interface for the joint survey inversion
...
This is particularly important when the surveys have different units or sensitivities.
...
where :math:`\delta \beta` is the chosen cooling factor of the trade-off parameter, and the chi factor :math:`\chi_j` for each survey is computed as,

.. math::

    \chi_j = \frac{\phi_d^j}{N_j} \,,

the ratio between the data misfit and the number of data points.
The survey with the highest chi factor will have a scaling factor of 1. The other surveys will have a scaling factor between :math:`\delta \beta` and 1, depending on how far their chi factor is from the maximum chi factor.
In other words, misfits with the lowest chi factors are scaled down by the same amount as the trade-off parameter, effectively reducing their contribution to the model update.
As the inversion progresses and the chi factors of all surveys reach a similar value, the scaling factors will increase towards 1, allowing all surveys to contribute more equally to the model update.
