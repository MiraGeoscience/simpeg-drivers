...
the model function :math:`f(\mathbf{m})`.
...
The `figure above <sparse_model>`__ shows vertical sections through the
true and recovered models (from left to right) with L2, L1 and L0-norm
on the model values.

Note that as :math:`p \rightarrow 0` the volume of the anomaly shrinks
to a few non-zero elements while the physical property contrasts
increase. They generally agree on the center position of the anomaly but
differ significantly in their extent and shape.

.. figure:: ./images/smallness_norm_setup.png
   :alt: smallness_setup

   smallness_setup

No smoothness constraints were used (:math:`\alpha_{x,y,z} = 0`), only a
penalty on the reference model (:math:`m_{ref} = 0`).

All those models fit the data to the target `data
misfit <data-misfit>`__ and are therefore valid solutions to the
inversion.
...
The `figure above <flat_model>`__ shows vertical sections through the
true and recovered models (from left to right) with L2, L1 and L0-norm
on the model gradients.

.. figure:: ./images/smoothness_norm_setup.png
   :alt: smoothness_setup

   smoothness_setup

No reference model was used (:math:`\alpha_s = 0`) with uniform norm
values on all three Cartesian components.

All those models also fit the data to the target `data
misfit <data-misfit>`__ and are therefore valid solutions to the
inversion. Note that as :math:`p \rightarrow 0` the edges of the anomaly
become tighter while variability inside the body diminishes. They
generally agree on the center position of the anomaly but differ greatly
on the extent and shape.
...
bounded by all possible combinations on :math:`[0, 2]` applied to each
function independently.

To be continued…

Background: Approximated :math:`L_p`-norm
-----------------------------------------

The standard :math:`L_p` norm measure is generally written as
...
For :math:`p<=1`, the function is non-linear with respect to the model.
It is possible to approximate the function in a linearized form as
...
where :math:`p` is any constant between :math:`[0,\;2]`. This is a
decent approximation of the :math:`l_p`-norm for any of the functions
presented in the `L2-norm <l2-norm>`__ section.

.. figure:: ./images/lp_norm.png
   :alt: lp_norm

   lp_norm

Note that choosing a different :math:`l_p`-norm greatly changes how we
measure the function :math:`f(m)`. Rather than simply increasing
exponentially with the size of :math:`f(m)`, small norms increase the
penalty on low :math:`f(m)` values. As :math:`p\rightarrow 0`, we
attempt to recover as few non-zero elements, which in turn favour sparse
solutions.

Since it is a non-linear function with respect to :math:`\mathbf{m}`, we
can linearize it as
...
Here, the superscript :math:`(k)` denotes the iteration step of the
inversion. This is also known as an iterative re-weighted least-squares
(IRLS) method. For more details on the implementation, refer to
...
slightly, the general :math:`l_p`-norm methodology presented above
encompasses this specific case. We can write
...
which recovers the total-variation (TV) function. Lastly, for
:math:`p=2`
...
more general Lp-norm regularization function for :math:`p \in [0, 2]`.
