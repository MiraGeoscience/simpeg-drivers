...
obtained with a dip rotation of $45\\circ$ towards East.*
...
![local_rotation](./images/local_rotation.png)
*Example of an inversion with local rotations along the expected limbs of a folded layer.*

## Input format

Direction information must be supplied as a `Data Group` defined on the
inversion mesh. The following group types are accepted:
`Dip direction & Dip`, `Strike & Dip`, or `3D vector`.

![set_orientation](./images/set_orientation.png)
...
![gradient_direction](./images/gradient_direction.png)


## Background

As covered in the [Model Smoothness](smooth-ref) section, penalties on
the model gradients are measured by:

$$\mathbf{f}_i = \mathbf{G}_i (\mathbf{m} - \mathbf{m}_{ref}),$$

where $\mathbf{G}_i$ is a finite difference operator that computes the
difference in model $\mathbf{m}$ values between adjacent cells along one
of the Cartesian directions: East (X), North (Y), and vertical (Z). As
proposed by {cite:t}`LiOldenburg2000`, orientation information can be
incorporated into the inversion by rotating the gradient operators along
arbitrary directions such that

$$\mathbf{\hat f}_i = \mathbf{\hat G}_i (\mathbf{m} - \mathbf{m}_{ref}),$$

where $\mathbf{\hat G}_i$ are rotated gradients along one of the $u$,
$v$, and $w$-axes.

![plane_rotation](./images/plane_rotation.png)
...
![partial_volumes](./images/partial_volumes.png)
...
![fold_model](./images/fold_model.png)
...
![fold_data](./images/fold_data.png)
...
example can be [downloaded here](https://github.com/MiraGeoscience/simpeg-drivers/raw/refs/heads/develop/simpeg_drivers-assets/tutorial_rotated_gradients.geoh5?download=).
...
![fold_inverted_no_rot](./images/fold_inverted_no_rot.png)
...
![fold_structural_interp](./images/fold_structural_interp.png)
...
To further accentuate the trend, the weight of the `Z-smoothness weight`
was decreased, as it is now oriented perpendicular to the plane of
rotation. By doing so, the relative strength of the smoothing (and
sparsity) along the local dip and strike of the magnetic layer is
increased.

![fold_parameters](./images/fold_parameters.png)
...
![old_inverted_rotated_wz0p1](./images/fold_inverted_rotated_wz0p1.png)
